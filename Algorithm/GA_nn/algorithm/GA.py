...
class GA(Algorithm):
    def __init__(self, problem, dimension, bounds, gene_num=10, n_generations=100, population_size=100):
    ...
    def decode(self, codes):
        array = []
        w1_b1 = np.array(codes[:n_neurons_1]).T
        w2_b2 = np.array(codes[n_neurons_1:]).T
        b1 = w1_b1[0, :].reshape(n_neurons_1, 1)
        b2 = w2_b2[0, :].reshape(1, 1)
        w1 = w1_b1[1:, :]
        w2 = w2_b2[1:, :]
        array = [[w1, w2], [b1, b2]]
        return array
    ...
    def init(self, example=None):
        self.generation = 0
        self.population_DNA = [[[random.uniform(-1, 1) for i in range(self.gene_num[j])]
